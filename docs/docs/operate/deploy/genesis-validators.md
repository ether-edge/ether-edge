...
This can be done using the `ether-edge polybft whitelist-validators` command. The deployer can specify the hex-encoded private key of the `CustomSupernetManager` contract deployer or use the `--data-dir` flag if they have initialized their secrets.
...
| `--data-dir`        | Directory for the Polygon Edge data if the local FS is used                                     | `--data-dir ./ether-edge/data`             |
...
Each validator needs to register themselves on the `CustomSupernetManager` contract. This is done using the `ether-edge polybft register-validator` command. **Note that this command is for testing purposes only.**
...
Each validator needs to perform initial staking on the rootchain `StakeManager` contract. This is done using the `ether-edge polybft stake` command. **Note that this command is for testing purposes only.**
