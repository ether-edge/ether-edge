...
build: check-go check-git
	...
	go build -o ether-edge -ldflags="\
    	-X 'github.com/ether-edge/ether-edge/versioning.Version=$(VERSION)' \
		-X 'github.com/ether-edge/ether-edge/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/ether-edge/ether-edge/versioning.Branch=$(BRANCH)'\
		-X 'github.com/ether-edge/ether-edge/versioning.BuildTime=$(TIME)'" \
	main.go
...
.PHONY: test-e2e
test-e2e: check-go
	go build -race -o artifacts/ether-edge .
	env EDGE_BINARY=${PWD}/artifacts/ether-edge go test -v -timeout=30m ./e2e/...

.PHONY: test-e2e-polybft
test-e2e-polybft: check-go
	go build -o artifacts/ether-edge .
	env EDGE_BINARY=${PWD}/artifacts/ether-edge E2E_TESTS=true E2E_LOGS=true \
	go test -v -timeout=1h30m ./e2e-polybft/e2e/...

.PHONY: test-property-polybft
test-property-polybft: check-go
	go build -o artifacts/ether-edge .
	env EDGE_BINARY=${PWD}/artifacts/ether-edge E2E_TESTS=true E2E_LOGS=true go test -v -timeout=30m ./e2e-polybft/property/... \
	-rapid.checks=10
